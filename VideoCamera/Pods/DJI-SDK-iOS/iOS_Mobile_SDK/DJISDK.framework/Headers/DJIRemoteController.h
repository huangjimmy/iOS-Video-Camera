 ...
 *  The remote  controller will start to update the progress after the calibration
 ...
 *  one device (e.g. aircraft, RTK base station) at  the same time. When this
 *  feature is supported,  use `startMultiDevicePairingWithTarget:withCompletion`
 *  and `stopMultiDevicePairingWithCompletion` to start and stop  pairing instead of
 ...
 *  enable/disable the channel when the aircraft is flying. Only Supported by
 *  Phantom 4 RTK.
 *  
 ...
 *  `getCalibrationComponentsWithCompletion`) should be  returned to their
 *  midpoints. The calibration progress for each component can be received  from
