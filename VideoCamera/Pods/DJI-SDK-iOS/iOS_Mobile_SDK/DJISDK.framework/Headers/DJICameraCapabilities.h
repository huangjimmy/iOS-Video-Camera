 ...
 *  `DJICameraPhotoTimeIntervalSettings`.
 *  
 *  @return Array of NSNumber. Each element represent one current supported camera photo count range.
 */
 ...
 *  `DJICameraThermalPalette`).  Returns `nil` if  current thermal camera has not
 ...
 *  `DJICameraTemperatureUnit`).  Returns `nil` if  current thermal camera has not
