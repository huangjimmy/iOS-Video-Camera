 ...
 *  flying to the start point but it will not continue to circle around the hotpoint
 *  until the user resumes the mission. After a mission is paused successfully, the
 *  `currentState` will become `DJIHotpointMissionStateExecutionPaused`.
 *  
 ...
 *   - `DJIHotpointMissionStateExecutionPaused`
 *  
 *  @param completion Completion block that will be called when the operator succeeds or fails to reset the mission heading. If it fails, an error will be returned.
 */
