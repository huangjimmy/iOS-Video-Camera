...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        // add it to the view
        imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"battery.png"]];
        ...
        batteryLabel = [[UILabel alloc] initWithFrame:self.bounds];
...
- (void)setBatteryLevelWithAnimation:(BOOL)isAnimated forValue:(CGFloat)batteryLevel inPercent:(BOOL)inPercent {
    ...
    if (isAnimated)
        [UIView animateWithDuration:2.0 animations:^{
            ...
            self->batteryFill.backgroundColor = [self setColorBasedOnBatteryLevel:newBatteryLevel];
        }];
    else
